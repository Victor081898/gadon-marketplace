...
const lots = [
  { id: 1, name: 'Nelore selecionado', meta: '80 machos · 12@', weight: 12, price: 'R$ 28.000', unit: 'R$ 2.333,33 / cabeça', place: 'Campo Verde - MT', category: 'Nelore', age: '18 a 24 meses', seller: 'Fazenda Santa Rita', image: 'https://images.unsplash.com/photo-1551884831-bbf3cdc6469e?auto=format&fit=crop&w=900&q=85', accent: 'blue' },
  { id: 2, name: 'Angus premium', meta: '50 fêmeas · 10@', weight: 10, price: 'R$ 23.500', unit: 'R$ 2.350,00 / cabeça', place: 'Dourados - MS', category: 'Angus', age: '20 a 28 meses', seller: 'Agro Boa Vista', image: 'https://images.unsplash.com/photo-1500595046743-cd271d694d30?auto=format&fit=crop&w=900&q=85', accent: 'orange' },
  { id: 3, name: 'Nelore matriz', meta: '120 matrizes · 15@', weight: 15, price: 'R$ 45.600', unit: 'R$ 2.280,00 / cabeça', place: 'Aparecida do Taboado - MS', category: 'Nelore', age: '24 a 36 meses', seller: 'Fazenda JP', image: 'https://images.unsplash.com/photo-1527153857715-3908f2bae5e8?auto=format&fit=crop&w=900&q=85', accent: 'green' },
  { id: 4, name: 'Cruza industrial', meta: '60 machos · 10@', weight: 10, price: 'R$ 19.800', unit: 'R$ 1.980,00 / cabeça', place: 'Goiânia - GO', category: 'Cruza', age: '16 a 22 meses', seller: 'Fazenda São Miguel', image: 'https://images.unsplash.com/photo-1596733430284-f7437764b1a9?auto=format&fit=crop&w=900&q=85', accent: 'purple' },
  { id: 5, name: 'Bezerros Nelore', meta: '40 machos · 8@', weight: 8, price: 'R$ 11.000', unit: 'R$ 2.750,00 / cabeça', place: 'Rondonópolis - MT', category: 'Bezerros', age: '8 a 12 meses', seller: 'Fazenda Horizonte', image: 'https://images.unsplash.com/photo-1516467508483-a7212febe31a?auto=format&fit=crop&w=900&q=85', accent: 'blue' },
  { id: 6, name: 'Lote de reposição', meta: '30 fêmeas · 8@', weight: 8, price: 'R$ 6.900', unit: 'R$ 2.300,00 / cabeça', place: 'Campo Grande - MS', category: 'Outros', age: '12 a 18 meses', seller: 'Estância Boa Água', image: 'https://images.unsplash.com/photo-1546445317-29f4545e9d53?auto=format&fit=crop&w=900&q=85', accent: 'orange' },
];
...
const state = { activeNav: 'Início', query: '', category: 'Todos', collectionView: 'all', favorites: loadFavorites(), selectedLots: new Set(), filterOpen: false, advancedFilters: { region: 'Todos', minWeight: '', maxWeight: '' }, sort: 'relevance', lotHistory: loadLotHistory(), modalLot: null, modalTab: 'description', modalMediaIndex: 0, toast: '', page: 'home', auditLog: loadAuditLog(), messages: loadMessages(), activeConversationId: 1, messageQuery: '', recording: false, freightTrips: loadFreightTrips(), freightCalendarOpen: false, freightDocuments: loadFreightDocuments(), freightDocumentsOpen: false, calendarYear: 2026, calendarMonth: 6, notifications: loadNotifications(), notificationsOpen: false };
...
function getFilteredLots() {
  ...
  const { region, minWeight, maxWeight } = state.advancedFilters;
  const filtered = lots.filter((lot) => {
    const searchable = `${lot.name} ${lot.place} ${lot.category} ${lot.seller}`.toLowerCase();
    ...
    return matchesQuery && matchesCategory && matchesRegion && matchesMin && matchesMax && matchesCollection;
  });
  ...
  document.querySelector('#app').innerHTML = `
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <div class="profile-mini"><div class="avatar">JP</div><div><strong>João Pecuarista</strong><span>Comprador verificado</span></div><button class="icon-button">${icon('chevron', 15)}</button></div>
        <nav class="main-nav">
          <p class="nav-label">MENU PRINCIPAL</p>
          ${['Início', 'Buscar gado', 'Meus anúncios', 'Mensagens', 'Fretes', 'Carga de retorno'].map((item, i) => `<button class="nav-item ${state.activeNav === item ? 'active' : ''}" data-nav="${item}">${icon(['home','search','cow','message','truck','repeat'][i])}<span>${item}</span>${item === 'Mensagens' && state.messages.reduce((sum, conversation) => sum + (conversation.unread || 0), 0) ? `<b>${state.messages.reduce((sum, conversation) => sum + (conversation.unread || 0), 0)}</b>` : ''}</button>`).join('')}
      ...
      <main class="main-content">
        ...
        <div class="page-body">
          ...
          <section class="operations"><div class="operation-panel freight"><div class="operation-icon">${icon('truck', 22)}</div><div><p class="eyebrow">FRETE PARCEIRO</p><h3>Leve seu gado com segurança.</h3><p>Solicite cotações de transportadoras parceiras para sua rota.</p><button class="outline-button" data-nav="Fretes">Cotar frete ${icon('arrow', 15)}</button></div><div class="route-lines"></div></div><div class="operation-panel return"><div class="operation-icon">${icon('repeat', 22)}</div><div><p class="eyebrow">INTELIGÊNCIA LOGÍSTICA</p><h3>Seu caminhão pode voltar carregado.</h3><p>Encontre oportunidades de carga de retorno na sua região.</p><button class="outline-button" data-nav="Carga de retorno">Ver oportunidades ${icon('arrow', 15)}</button></div><div class="mini-route"><span>GO</span><i></i><span>MT</span></div></div></section>
...
function bindEvents() {
  document.querySelectorAll('[data-nav]').forEach((el) => el.addEventListener('click', () => { state.activeNav = el.dataset.nav; if (el.dataset.nav === 'Mensagens') state.page = 'messages'; else if (el.dataset.nav === 'Fretes') state.page = 'freight'; else if (el.dataset.nav === 'Buscar gado') { state.page = 'search'; state.collectionView = 'all'; } else { state.page = 'home'; } render(); }));
  ...
  document.querySelector('[data-filter-action="reset"]')?.addEventListener('click', () => { state.advancedFilters = { region: 'Todos', minWeight: '', maxWeight: '' }; state.filterOpen = false; render(); });
  document.querySelector('#advanced-filters')?.addEventListener('submit', (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget).entries()); state.advancedFilters = { region: data.region || 'Todos', minWeight: data.minWeight || '', maxWeight: data.maxWeight || '' }; state.filterOpen = false; render(); });
  document.querySelector('#lot-sort')?.addEventListener('change', (event) => { state.sort = event.target.value; render(); });
  document.querySelectorAll('[data-account-view]').forEach((el) => el.addEventListener('click', () => { state.collectionView = el.dataset.accountView; state.activeNav = 'Início'; state.page = 'home'; render(); }));
  ...
  document.querySelectorAll('[data-action="buy-lot"]').forEach((el) => el.addEventListener('click', () => beginLotNegotiation(state.modalLot, true)));
  document.querySelector('[data-action="clear-selection"]')?.addEventListener('click', () => { state.selectedLots.clear(); render(); });
  document.querySelector('[data-action="buy-selected"]')?.addEventListener('click', () => { const selected = lots.filter((lot) => state.selectedLots.has(lot.id)); beginLotNegotiation(selected[0], true, selected); });
...
function messagesTemplate() {
  ...
  return `<div class="app-shell messages-shell">
    <aside class="sidebar"><div class="brand"><div class="brand-mark"><img src="/gadon.jpeg" alt="" /></div><div><strong>GAD<span>O</span>N</strong><small>O mercado do Gado</small></div></div><div class="profile-mini"><div class="avatar">JP</div><div><strong>João Pecuarista</strong><span>Comprador verificado</span></div><button class="icon-button">${icon('chevron', 15)}</button></div><nav class="main-nav"><p class="nav-label">MENU PRINCIPAL</p>${['Início', 'Buscar gado', 'Meus anúncios', 'Mensagens', 'Fretes', 'Carga de retorno'].map((item, i) => `<button class="nav-item ${state.activeNav === item ? 'active' : ''}" data-nav="${item}">${icon(['home','search','cow','message','truck','repeat'][i])}<span>${item}</span>${item === 'Mensagens' && unreadCount ? `<b>${unreadCount}</b>` : ''}</button>`).join('')}<p class="nav-label nav-spacer">CONTA</p><button class="nav-item">${icon('heart')}<span>Favoritos</span></button><button class="nav-item">${icon('user')}<span>Meu perfil</span></button></nav><div class="sidebar-bottom"><div class="help-card"><div class="help-icon">?</div><div><strong>Precisa de ajuda?</strong><span>Fale com nosso suporte</span></div>${icon('chevron', 15)}</div><div class="sidebar-foot">GadOn <span>•</span> versão 1.0 MVP</div></div></aside>
...
function bindMessagesEvents() {
  document.querySelectorAll('[data-nav]').forEach((el) => el.addEventListener('click', () => { state.activeNav = el.dataset.nav; state.page = el.dataset.nav === 'Mensagens' ? 'messages' : el.dataset.nav === 'Fretes' ? 'freight' : el.dataset.nav === 'Buscar gado' ? 'search' : 'home'; if (el.dataset.nav === 'Buscar gado') state.collectionView = 'all'; render(); }));
  document.querySelectorAll('[data-action="register"]').forEach((el) => el.addEventListener('click', () => { state.page = 'register'; state.toast = ''; render(); }));
...
function bindFreightEvents() {
  ...
  document.querySelector('[data-freight-action="documents"]')?.addEventListener('click', () => { state.freightDocumentsOpen = true; state.freightCalendarOpen = false; render(); });
  ...
  document.querySelector('#freight-document-form')?.addEventListener('submit', (event) => { event.preventDefault(); const file = document.querySelector('#freight-document-file')?.files?.[0]; if (!file) return; if (file.size > 10 * 1024 * 1024) { showToast('O documento deve ter no máximo 10 MB.'); return; } const data = Object.fromEntries(new FormData(event.currentTarget).entries()); const reference = data.reference || file.name.replace(/\.[^.]+$/, ''); state.freightDocuments.unshift({ id: Date.now(), type: data.type, name: reference, trip: data.trip, status: 'Enviado', statusClass: 'issued', fileName: file.name, fileType: file.type || 'application/octet-stream', size: file.size, expiresAt: data.expiresAt || '', notes: data.notes || '', uploadedAt: new Intl.DateTimeFormat('pt-BR').format(new Date()) }); saveFreightDocuments(); state.freightDocumentsOpen = false; showToast('Documento adicionado à gestão de documentos.'); });
...
function bindRegistrationEvents() {
  document.querySelectorAll('[data-action="back-home"]').forEach((el) => el.addEventListener('click', () => { state.page = 'home'; state.toast = ''; render(); }));
  document.querySelector('#cattle-form')?.addEventListener('submit', (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget).entries()); saveAuditLog(createRegistrationLog(data)); state.toast = 'Lote habilitado e enviado para análise.'; render(); setTimeout(() => { state.toast = ''; render(); }, 3600); });
}
