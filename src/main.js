...
const lots = [
  { id: 1, name: 'Nelore selecionado', meta: '80 machos · 12@', weight: 12, price: 'R$ 28.000', unit: 'R$ 2.333,33 / cabeça', place: 'Campo Verde - MT', category: 'Nelore', sex: 'Machos', age: '18 a 24 meses', ageMonths: 21, purpose: 'Engorda', seller: 'Fazenda Santa Rita', image: '/nelore-selecionado.png', accent: 'blue' },
...
function updateRegistrationLog(id, data, attachments = {}) {
  const current = state.auditLog.find((record) => record.id === id && record.lot);
  if (!current) return false;
  const replacement = createRegistrationLog(data, attachments);
  const now = new Date().toISOString();
  current.status = 'EM_VERIFICACAO';
  current.updatedAt = now;
  current.lot = replacement.lot;
  current.steps = [{ label: 'Cadastro atualizado', status: 'completed', at: now }, ...(current.steps || []).filter((step) => step.label !== 'Cadastro atualizado').map((step) => step.label === 'Análise de documentos' ? { ...step, status: 'current', at: null } : step)];
  try { localStorage.setItem(auditStorageKey, JSON.stringify(state.auditLog)); } catch { /* armazenamento local indisponível */ }
  return true;
}
...
function createRegistrationLog(data, attachments = {}) {
  const now = new Date();
  return {
...
function announcementStatusLabel(status) {
  return ({ EM_VERIFICACAO: 'Em verificação', APROVADO: 'Aprovado', REPROVADO: 'Ajustes necessários', REGISTRADO: 'Registrado' }[status] || status || 'Registrado');
}

function announcementRecordTemplate(record) {
  if (!record?.lot) return '';
  const lot = record.lot;
  const value = (item, fallback = 'Não informado') => escapeHtml(item || fallback);
  const list = (items, empty) => Array.isArray(items) && items.length ? items.map((item) => `<span>${icon('file', 13)} ${escapeHtml(item)}</span>`).join('') : `<small>${escapeHtml(empty)}</small>`;
  const steps = Array.isArray(record.steps) ? record.steps : [];
  const status = announcementStatusLabel(record.status);
  const availableAt = lot.availableAt ? new Intl.DateTimeFormat('pt-BR').format(new Date(`${lot.availableAt}T12:00:00`)) : 'A combinar';
  return `<div class="announcement-record-backdrop" data-close-announcement-record><section class="announcement-record-modal" role="dialog" aria-modal="true" aria-labelledby="announcement-record-title"><button type="button" class="modal-close" data-action="close-announcement-record" aria-label="Fechar registro">${icon('close', 18)}</button><header class="announcement-record-head"><div><p class="eyebrow">REGISTRO DO PRODUTO</p><h2 id="announcement-record-title">${value(lot.name, 'Lote sem nome')}</h2><p>Consulta completa do cadastro enviado pelo vendedor.</p></div><span class="announcement-record-status">${escapeHtml(status)}</span></header><div class="announcement-record-protocol"><div><span>PROTOCOLO</span><strong>${value(record.id)}</strong></div><div><span>CADASTRADO EM</span><strong>${record.createdAt ? escapeHtml(formatAuditDate(record.createdAt)) : 'Data não informada'}</strong></div></div><section class="announcement-record-section"><div class="announcement-record-section-head"><span>${icon('cow', 17)}</span><div><p class="eyebrow">DADOS DO LOTE</p><h3>Características comerciais</h3></div></div><div class="announcement-record-facts"><div><span>Espécie</span><strong>${value(lot.species)}</strong></div><div><span>Raça predominante</span><strong>${value(lot.breed)}</strong></div><div><span>Composição racial</span><strong>${value(lot.composition)}</strong></div><div><span>Finalidade</span><strong>${value(lot.purpose)}</strong></div><div><span>Quantidade</span><strong>${value(lot.quantity ? `${lot.quantity} cabeças` : '')}</strong></div><div><span>Sexo predominante</span><strong>${value(lot.sex)}</strong></div><div><span>Idade média</span><strong>${value(lot.age ? `${lot.age} meses` : '')}</strong></div><div><span>Peso médio</span><strong>${value(lot.weight ? `${lot.weight} @` : '')}</strong></div><div><span>Preço total</span><strong>${value(lot.price)}</strong></div><div><span>Retirada disponível</span><strong>${value(availableAt)}</strong></div></div>${lot.description ? `<div class="announcement-record-description"><span>Observações</span><p>${value(lot.description)}</p></div>` : ''}</section><section class="announcement-record-section"><div class="announcement-record-section-head"><span>${icon('pin', 17)}</span><div><p class="eyebrow">ORIGEM E PROPRIEDADE</p><h3>Procedência informada</h3></div></div><div class="announcement-record-facts"><div><span>Fazenda</span><strong>${value(lot.farm)}</strong></div><div><span>Origem</span><strong>${value(lot.origin)}</strong></div><div><span>Registro da propriedade</span><strong>${value(lot.propertyCode)}</strong></div><div><span>Distância até a rodovia</span><strong>${value(lot.roadDistance ? `${lot.roadDistance} km` : '')}</strong></div></div></section><section class="announcement-record-section"><div class="announcement-record-section-head"><span>${icon('shield', 17)}</span><div><p class="eyebrow">SANIDADE E DOCUMENTAÇÃO</p><h3>Itens declarados no cadastro</h3></div></div><div class="announcement-record-facts"><div><span>Situação sanitária</span><strong>${value(lot.healthStatus)}</strong></div><div><span>Rastreabilidade</span><strong>${value(lot.traceability)}</strong></div><div><span>GTA</span><strong>${value(lot.gtaStatus)}</strong></div><div><span>Certificado / atestado</span><strong>${value(lot.certificate)}</strong></div></div><div class="announcement-record-files"><div><span>Fotos do lote</span>${list(lot.photoNames, 'Nenhuma foto registrada')}</div><div><span>Documentos de apoio</span>${list(lot.documentNames, 'Nenhum documento registrado')}</div></div><p class="announcement-record-disclaimer">Este registro representa as informações declaradas pelo vendedor. A validação definitiva de GTA, certificados, propriedade e demais documentos depende da análise do GadOn e dos órgãos competentes.</p></section><section class="announcement-record-section announcement-record-audit"><div class="announcement-record-section-head"><span>${icon('clock', 17)}</span><div><p class="eyebrow">LINHA DO TEMPO</p><h3>Acompanhamento da verificação</h3></div></div><div class="announcement-record-timeline">${steps.length ? steps.map((step, index) => `<div class="announcement-record-step ${escapeHtml(step.status || 'pending')}"><i>${step.status === 'completed' ? '✓' : index + 1}</i><div><strong>${value(step.label)}</strong>${step.at ? `<small>${escapeHtml(formatAuditDate(step.at))}</small>` : '<small>Aguardando atualização</small>'}</div></div>`).join('') : '<p class="announcement-record-empty">Nenhum evento de auditoria registrado.</p>'}</div></section><footer class="announcement-record-footer"><span>${icon('shield', 14)} Registro local para acompanhamento do processo.</span><button type="button" class="secondary-button" data-action="close-announcement-record">Fechar registro</button></footer></section></div>`;
}

function selectedAnnouncementRecord() {
  return state.announcementRecordId ? getUserAnnouncements().find((record) => record.id === state.announcementRecordId) : null;
}

function openAnnouncementRecord(id) {
  if (!getUserAnnouncements().some((record) => record.id === id)) { showToast('Produto não encontrado no catálogo.'); return; }
  state.announcementRecordId = id;
  render();
}

function bindAnnouncementRecordEvents() {
  document.querySelectorAll('[data-announcement-protocol]').forEach((el) => el.addEventListener('click', () => {
    const id = el.dataset.announcementProtocol || '';
    openAnnouncementRecord(id);
  }));
...
function bindSellerMarketplaceEvents() {
  ...
  document.querySelectorAll('[data-seller-action="products"]').forEach((el) => el.addEventListener('click', () => { state.activeNav = 'Meus produtos'; render(); setTimeout(() => document.querySelector('.seller-products-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0); }));
  document.querySelectorAll('[data-seller-action="view-product"]').forEach((el) => el.addEventListener('click', () => openAnnouncementRecord(el.dataset.announcementId || '')));
  document.querySelectorAll('[data-seller-action="edit-product"]').forEach((el) => el.addEventListener('click', () => openCattleRegistration(el.dataset.announcementId || '')));
...
function registrationTemplate() {
  const editingRecord = state.editingAnnouncementId ? getUserAnnouncements().find((record) => record.id === state.editingAnnouncementId) : null;
  const isEditing = Boolean(editingRecord);
  const registrationHeading = isEditing ? 'Edite os dados do seu gado.' : 'Cadastre os dados do seu gado.';
  const registrationButton = isEditing ? 'Salvar alterações' : 'Habilitar lote';
  return `<div class="register-shell">
    ...
    <main class="register-content">
      ...
      <div class="register-intro"><div><p class="eyebrow">${isEditing ? 'EDITAR PRODUTO ·' : 'HABILITAR LOTE ·'} ETAPA 1 DE 1</p><h1>${registrationHeading}</h1><p>${isEditing ? 'Atualize as informações do produto e reenvie o lote para análise.' : 'Preencha as informações abaixo para publicar seu lote e começar a receber contatos de compradores.'}</p></div><div class="progress-block"><div class="progress-label"><span>Progresso do cadastro</span><strong>25%</strong></div><div class="progress-track"><i></i></div></div></div>
      <div class="register-layout">
        <form id="cattle-form" class="registration-form" ${editingRecord ? `data-editing-announcement="${escapeHtml(editingRecord.id)}"` : ''}>
          ...
          <div class="register-footer"><span><b>*</b> Campos obrigatórios</span><button type="button" class="secondary-button" data-action="back-home">Cancelar</button><button type="submit" class="primary-button">${registrationButton} ${icon('arrow', 15)}</button></div>
        </form>
...
function hydrateRegistrationForm(record) {
  if (!record?.lot) return;
  const lot = record.lot;
  const values = { lotName: lot.name, species: lot.species, purpose: lot.purpose, breed: lot.breed, composition: lot.composition, quantity: lot.quantity, sex: lot.sex, age: lot.age, weight: lot.weight, price: lot.price, availableAt: lot.availableAt, description: lot.description, farm: lot.farm, city: lot.city, state: lot.state, propertyCode: lot.propertyCode, roadDistance: lot.roadDistance, healthStatus: lot.healthStatus, traceability: lot.traceability, gtaStatus: lot.gtaStatus, certificate: lot.certificate };
  Object.entries(values).forEach(([name, value]) => {
    const field = document.querySelector(`#cattle-form [name="${name}"]`);
    if (field && value !== undefined && value !== null) field.value = value;
  });
  document.querySelectorAll('#cattle-form .check-row input[type="checkbox"]').forEach((field) => { field.checked = true; });
}

function bindRegistrationEvents() {
  document.querySelector('input[name="photos"]')?.addEventListener('change', handleRegistrationPhotos);
  hydrateRegistrationForm(state.editingAnnouncementId ? getUserAnnouncements().find((record) => record.id === state.editingAnnouncementId) : null);
  ...
  document.querySelector('#cattle-form')?.addEventListener('submit', (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget).entries()); const currentRecord = state.editingAnnouncementId ? getUserAnnouncements().find((record) => record.id === state.editingAnnouncementId) : null; const fileNames = (name) => [...event.currentTarget.querySelector(`input[name="${name}"]`)?.files || []].map((file) => file.name); const documentNames = fileNames('documents').length ? fileNames('documents') : currentRecord?.lot?.documentNames || []; const attachments = { photoNames: state.registrationPhotos.map((photo) => photo.name), photos: state.registrationPhotos, documentNames }; const updated = currentRecord ? updateRegistrationLog(currentRecord.id, data, attachments) : false; if (!updated) saveAuditLog(createRegistrationLog(data, attachments)); state.registrationPhotos = []; state.editingAnnouncementId = null; state.toast = updated ? 'Produto atualizado e reenviado para análise.' : 'Lote habilitado e enviado para análise.'; state.page = state.mode === 'seller' ? 'sellerMarketplace' : 'home'; render(); setTimeout(() => { state.toast = ''; render(); }, 3600); });
}
...
function bindLotEvents() {
  ...
  document.querySelectorAll('[data-lot], [data-lot-photo]').forEach((el) => el.addEventListener('click', () => { const lot = lots.find((item) => item.id === Number(el.dataset.lot ?? el.dataset.lotPhoto)); if (!lot) return; state.modalLot = lot; state.modalTab = 'description'; state.modalMediaIndex = 0; state.lotHistory = [lot.id, ...state.lotHistory.filter((id) => id !== lot.id)].slice(0, 10); saveLotHistory(); render(); }));
  document.querySelectorAll('[data-lot-photo]').forEach((el) => el.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); el.click(); } }));
}
