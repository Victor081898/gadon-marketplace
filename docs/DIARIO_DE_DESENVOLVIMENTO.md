...
### 2026-08-01 — Pacote de demonstração para o evento (leilão ao vivo, loja completa, radar de fretes)
- Responsável: IA: Claude, front-end, produto e infra de demo.
- Objetivo: preparar o sistema completo para a apresentação de captura de leads: boas-vindas, cadastro por voz, leilão com transmissão, loja rural com checkout e mapas logísticos reais.
- Alterações: pop-up de boas-vindas com logo e CTA de cadastro; captura de leads em Supabase temporário (tabela `gadon_leads`, escrita anônima apenas) em conta, cadastro de gado, lances, transmissões, pedidos e solicitações de retorno; cadastro de gado com preenchimento por voz em tempo real (Web Speech API pt-BR) e câmera para fotos; leilão ao vivo com vídeo dos lotes (Mixkit em `public/videos/`), troca de câmeras, relógio, ficha técnica com lance ao vivo, lances simulados com anti-sniping e transmissão da câmera do próprio vendedor ("Leiloar meu lote"); loja rural completa com sidebar de categorias, busca, novas categorias (queijos, laticínios, mel), cadastro de produto do usuário persistido localmente e checkout em três etapas (entrega, pagamento com Pix/boleto/cartão, revisão com número de pedido); Radar de Fretes e página de Fretes de retorno reconstruída com mapa OSM real, rotas rodoviárias via OSRM com cache local, geolocalização com fallback, alertas de oportunidade e fichas com solicitação; normalização do design system (tokens de raio, sombra, espaçamento responsivo e containers), sidebar fixa compacta no desktop e topbar fixa no mobile; correção do worker do maplibre-gl no Vite via `setWorkerUrl` + import `?worker&url`.
- Arquivos: `src/main.js`, `src/styles.css`, `public/videos/*`, `package-lock.json`, `docs/DIARIO_DE_DESENVOLVIMENTO.md`.
- Contratos afetados: front-end passa a gravar leads via REST anônimo no projeto Supabase temporário do evento (somente INSERT); nenhuma API própria criada. Mapas dependem de tiles OSM e roteamento OSRM públicos (requer internet; rotas ficam em cache no navegador).
...
- Validação: `api/test/e2e.mjs` com 71 verificações verdes contra a API local (contas, anúncios, moderação, perguntas, favoritos, compra, conversas, notificações, documentos privados, pesador, pré-cadastro, fretes, permissões); `npm run build` verde; fluxos testados no navegador em 1280 px e 375 px, claro e escuro: anunciar (com foto), editar, aprovar, perguntar/responder, bloquear contato, pausar/reativar, perfil vendedor com documento, aprovação do vendedor, compra com frete e pagamento, conversa, confirmação de entrega, Radar, pesador, redefinição de senha, administração e central de fretes.
- Lotes de demonstração: selo "Demonstração" no card e na página do lote; a compra deles é simulada e não os retira do marketplace; com um provedor de pagamento real a API bloqueia a compra.
- Publicação (autorizada pelo responsável):
  - Backup do D1 de produção antes das migrações; migrações 0002–0004 aplicadas em produção e os 6 lotes de demonstração carregados.
  - API publicada em `api.gadon.com.br` e app em `app.gadon.com.br` (Pages, produção `main`).
