...
### 2026-07-28 — Infraestrutura de testes automatizados e primeiras suítes dos fluxos críticos
- Responsável: Humano: Pablo Figueiredo (`@ricardopablo1914-create`), com apoio de IA (Claude Code + agentes do `agent-mod`, papel `test-engineer`)
- Objetivo: atender as pendências de automação e testes registradas neste diário (busca e seleção, filtros combinados, documentos de frete e tabela de rotas) criando a base de testes do projeto.
- Alterações: adicionada a infraestrutura Vitest + jsdom (`vitest.config.js`, `tests/setup.js` com isolamento por teste, `tests/helpers.js`) e 5 suítes com 61 testes de interface cobrindo o comportamento real do protótipo, sem alterar `src/main.js`: `tests/marketplace-home.spec.js` (14), `tests/busca-selecao.spec.js` (12), `tests/filtros-avancados.spec.js` (13), `tests/documentos-frete.spec.js` (12) e `tests/tabela-rotas.spec.js` (12). Scripts `npm test` e `npm run test:watch` no `package.json`; CI (`.github/workflows/build.yml`) agora roda os testes antes do build. Criado `docs/PLANO_DE_TESTES.md` mapeando a cobertura atual e as próximas ondas de testes de API/integração a partir do deck do produto. Correção de cadastro: o usuário GitHub do Pablo registrado como `@ricardopablo1914` não existe; a mesma correção para `@ricardopablo1914-create` foi aplicada em paralelo na `main` pelo Victor (commit `43f7358`), então esta branch já herda os documentos corrigidos.
- Possíveis bugs encontrados pelos testes (cobertos como testes de caracterização marcados com `// BUG?` — decidir entre corrigir o app ou manter o comportamento): (1) navegar para "Buscar gado" reseta query/categoria/filtros mas não a ordenação (`state.sort`) nem a seleção (`src/main.js:407`); (2) o título da página de busca ignora o chip de raça ativo e segue "Todos os lotes" (`src/main.js:245`); (3) rota sem status exibe o rótulo "Contratada" com a cor de "Programada" (`src/main.js:565`); (4) cancelar o modal "Adicionar documento" aberto a partir do gerenciador "Todos os documentos" não reabre o gerenciador; (5) menor: contador "Pendentes" usa `!== 'issued'` enquanto a aba "pending" usa igualdade exata (`src/main.js:553-555`).
- Arquivos: `vitest.config.js`, `tests/setup.js`, `tests/helpers.js`, `tests/*.spec.js` (5), `package.json`, `package-lock.json`, `.github/workflows/build.yml`, `README.md`, `docs/PLANO_DE_TESTES.md`, `docs/DIARIO_DE_DESENVOLVIMENTO.md`.
